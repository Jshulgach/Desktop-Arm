...
class Listener(Node):
    def __init__(self,
        ...
        self.logger('Starting up server on {} port {}'.format(server_address, self.port))
    ...
    def wait_for_client(self):
        ...
        if self.verbose: print('Waiting for a connection...')
        ...
        while True:
            try:
                ...
                while self.connected:
                    data = self.connection.recv(1024).decode("ascii") # Grab all available bytes
                    if len(data)>1: # Make sure there's data available
                        if self.verbose: print('received {!r}'.format(data))
                        self.publishTopicData(data)
                        
                    else:
                        self.logger("Disconnection from {}".format(client_address))
                        self.connected = False
